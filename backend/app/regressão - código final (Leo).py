...
df_excel = pd.read_excel('Base Expandir_Final.xlsx', sheet_name='4_Base_Regressao')
...
def imprimir_resultados(servico, m1, m2, y1, y2, agg, variaveis, elast, cpc, p_ideal):
    ...
    print(f"  Preço ideal (6%)     : R$ {p_ideal:,.0f}")
    print(f"  Preço ideal (10%)    : R$ {cpc/0.90:,.0f}")
    print(f"  Preço ideal (15%)    : R$ {cpc/0.85:,.0f}")
...
for sv, modelo, variaveis, inp in cenarios:
    ...
    print(f"  {sv:12s} | {desc}")
    print(f"              Custo total: R${ct:,.0f}  |  Custo/ct: R${cpc:,.0f}  |  Preço ideal 6%: R${pi:,.0f}\n")
    print(f'')
...
#
# 1. Substitua o arquivo base Base Expandir_Final.xlsx pela base real em formato pickle,
